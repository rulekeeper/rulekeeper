...
export function readController (router, controller, overrides = []) {
  _.each(controller, action => {
    ...
    if (middlewares.indexOf(rulekeeper.contextMiddlewareHandler) === -1) middlewares.push(rulekeeper.contextMiddlewareHandler);
    if (middlewares.indexOf(rulekeeper.accessControlMiddlewareHandler) === -1) middlewares.push(rulekeeper.accessControlMiddlewareHandler);
